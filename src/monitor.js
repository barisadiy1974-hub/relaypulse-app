...
// SSH istemcisinin ConnectionAttempts değeri. buildSshArgs modül seviyesinde
// bir fonksiyon (this yok), o yüzden ayar burada tutulur; Monitor constructor'ı
// ve updateSettings() günceller.
let _sshRetryCount = 2;
...
// Kullanicinin ayarladigi servis adlarini ve portlari kabuk degiskeni olarak
// scriptin basina yazar. Adlar kullanicidan geldigi icin filtreleniyor: sadece
// systemd birim adlarinda gecerli karakterler kalir, boylece tirnak kacisi ya da
// komut enjeksiyonu mumkun olmaz.
function buildWatchPrefix(cfg) {
  const svcs = (Array.isArray(cfg.watchServices) ? cfg.watchServices : [])
    .map(s => String(s).trim())
    .filter(s => /^[A-Za-z0-9@._-]+$/.test(s));
  const ports = (Array.isArray(cfg.watchPorts) ? cfg.watchPorts : [])
    .map(p => parseInt(p, 10))
    .filter(p => Number.isInteger(p) && p > 0 && p < 65536);
  const lines = [];
  if (svcs.length) lines.push(`WATCH_SVCS='${svcs.join(' ')}'`);
  if (ports.length) lines.push(`WATCH_PORTS_RE='${ports.join('|')}'`);
...
# WATCH_SVCS and WATCH_PORTS_RE are prepended by the app from settings.
# Fall back to the historical defaults if this script is ever run by hand.
[ -n "$WATCH_SVCS" ] || WATCH_SVCS='anon anon@default anyone anyone-relay tor-anon'
[ -n "$WATCH_PORTS_RE" ] || WATCH_PORTS_RE='9001|9030|9050|9051'
# First non-inactive service wins. For each configured name we also look for
...
function _ensurePwMaster(server) {
  ...
  const p = new Promise((resolve, reject) => {
    ...
    const deadline = setTimeout(() => {
      ...
      finish(new Error(`SSH master connection could not be established (16s): ${stderr.trim()}`));
    }, 16000);
...
// sshpass exit codes — translate the useful ones into readable hints.
function sshpassHint(code) {
  switch (code) {
    case 1: return 'sshpass: invalid parameter';
    case 2: return 'sshpass: conflicting arguments';
    case 3: return 'sshpass: usage error';
    case 4: return 'sshpass: unknown error (stderr was empty)';
    case 5: return 'Password is incorrect or sshd password authentication is disabled. Check /etc/ssh/sshd_config > PasswordAuthentication yes';
    case 6: return 'Host key is unknown or has changed. Connect once from Terminal and accept it with yes.';
    case 7: return 'sshpass: I/O error';
    case 8: return 'sshpass: expected prompt did not appear';
    default: return null;
  }
}
...
function normalizeSshError(text, server) {
  ...
  if (!msg) return msg;
  const host = server?.host || server?.sshAlias || server?.name || 'target';

  if (/Connection refused/i.test(msg)) {
    return `SSH refused: ${host}:${server?.port || 22}. SSH/sshd may be disabled on the remote server, or the firewall is refusing port 22.\n${msg}`;
  }
  if (/Permission denied/i.test(msg)) {
    return `Authentication failed: ${host}. Check the username, password, or key.\n${msg}`;
  }
  if (/Host key verification failed|REMOTE HOST IDENTIFICATION HAS CHANGED/i.test(msg)) {
    return `Host key mismatch: ${host}. Connect once manually from Terminal to fix the known_hosts entry.\n${msg}`;
  }
  if (/No route to host|Network is unreachable/i.test(msg)) {
    return `Network unreachable: ${host}. The server may be offline, or there may be a route/firewall problem.\n${msg}`;
  }
  if (/Broken pipe|send disconnect: Broken pipe|client_loop: send disconnect/i.test(msg)) {
    return `SSH connection dropped: ${host}. This is usually caused by a temporary network drop, NAT timeout, Wi-Fi change, or sshd restart.\n${msg}`;
  }
  if (/Connection closed by remote host|Connection reset by peer/i.test(msg)) {
    return `SSH session closed by the remote side: ${host}. The server, firewall, or an intermediate network may have dropped the session.\n${msg}`;
  }
  if (/SSH master (?:baglantisi kurulamadi|connection could not be established) \(16s\):\s*$/i.test(msg) || /banner exchange|kex_exchange_identification|Connection timed out during banner exchange/i.test(msg)) {
    return `SSH is not responding: ${host}:${server?.port || 22}. The port is open, but the sshd banner/handshake is not returning; the service may be stuck, overloaded, or held by a firewall.\n${msg}`;
  }
  return msg;
}
...
function runSshPasswordViaPrompt(server, remoteCmd, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out (${timeoutMs}ms). ${msg}`.trim()));
    }, timeoutMs);
...
function runSshPasswordOneshot(server, remoteCmd, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    execFile('sshpass', ['-e', 'ssh', ...args], {
      ...
      if (!err) return resolve(stdout.toString());
      if (err.code === 'ENOENT') {
        return reject(new Error('sshpass is not installed. Install it with: sudo apt-get install -y sshpass'));
      }
      const stderrText = (stderr || '').toString().trim();
      const errMsg = (err.message || '').trim();
      // execFile timeout: killed=true, signal SIGTERM, no exit code
      if (err.killed) {
        return reject(new Error(normalizeSshError(`Timed out (${timeoutMs}ms). ${stderrText}`, server)));
      }
...
function runSsh(server, remoteCmd, timeoutMs = 10000) {
  if (SANDBOX) return sandboxTransport.runSsh(server, remoteCmd, timeoutMs);
  ...
  if (server.password) {
    ...
    return passAttempt.catch((passErr) => {
      ...
      if (!/Permission denied|Authentication failed|Password is incorrect|password authentication is disabled|Parola HATALI|password auth kapali/i.test(msg)) throw passErr;
      ...
      return runSsh(keyServer, remoteCmd, timeoutMs).catch((keyErr) => {
        ...
        throw new Error(`${msg}\nSSH alias/key fallback also failed: ${keyMsg}`);
      });
    });
  }
  ...
  return new Promise((resolve, reject) => {
    const tryRun = (reuseControlMaster) => {
      ...
      const child = execFile('ssh', sshArgs, { timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024 }, (err, stdout, stderr) => {
        ...
        const rc = err.code;
        if (rc === 'ENOENT') {
          return reject(new Error('ssh command not found (PATH problem). ' + errMsg));
        }
        const isTimeout = err.killed || /ETIMEDOUT|timed out/i.test(errMsg);
        const rawErr = stderrText || errMsg || `rc=${rc}`;
        // ControlMaster socket'i bayatlamis olabilir — taze (mux'suz) baglantiyla
        // bir kez daha dene. AMA sadece gercekten gecici gorunen hatalarda.
        // ONCEKI BUG: her hataya "zaman asimi" prefix'i eklenip isTransientSshError'a
        // veriliyordu → her zaman true → "Permission denied" bile ikinci bir
        // baglanti denemesi tetikliyor, yanlis anahtarli relay poll basina 2 auth
        // hatasi uretip fail2ban'i 2x hizli doldurup Mac IP'sini banlatyordu.
        const authFail = /Permission denied|Host key verification failed|REMOTE HOST IDENTIFICATION HAS CHANGED|Too many authentication failures/i.test(rawErr);
        if (reuseControlMaster && !authFail && (isTimeout || isTransientSshError(rawErr))) {
          return tryRun(false);
        }
        const normalized = normalizeSshError(
          isTimeout ? `Timed out (${timeoutMs}ms). ${rawErr}` : rawErr,
          server,
        );
      ...
      child.on('error', (e) => {
        if (e.code === 'ENOENT') {
          return reject(new Error('ssh command not found (PATH problem). ' + e.message));
        }
  ...
  return new Promise((resolve, reject) => {
    ...
    const tryScheme = (idx, lastErr) => {
      if (idx >= schemes.length) {
        return reject(lastErr || new Error('Agent connection error'));
      }
      ...
      const opts = {
        ...
        path: '/metrics',
        method: 'GET',
        // 12s: iPhone uygulamasiyla ayni. 6s'te yogun anlarda agent gec kalip
        // SSH fallback'i tetikliyordu; SSH yolu da timeout verince sarı "stale"
        // alarm cikiyordu. Iki uygulamanin ayni esigi kullanmasi, "Mac'te alarm
        // yok ama iPhone'da var" tutarsizligini da kaldiriyor.
        timeout: 12000,
        headers: token ? { 'X-Agent-Token': token } : {},
      };
      if (scheme === 'https') {
        // Self-signed relay-local certs, pinned on first sight (src/cert-pin.js)
        // so the token is never handed to a different certificate.
        certPin.apply(opts, host);
      }
      const req = mod.request(opts, (res) => {
        if (scheme === 'https') certPin.learn(host, res);
        if (res.statusCode === 403) return reject(new Error('Agent token is invalid'));
        let body = '';
        res.on('data', d => { body += d; });
        res.on('end', () => {
          try { resolve(JSON.parse(body)); }
          catch (e) { reject(new Error('Agent JSON parse error: ' + e.message)); }
        });
      });
      req.on('timeout', () => {
        req.destroy();
        const err = new Error(`Agent ${scheme.toUpperCase()} timed out (6s)`);
        if (!configuredScheme && idx + 1 < schemes.length) return tryScheme(idx + 1, err);
        reject(err);
      });
      req.on('error', (e) => {
        // A refused pin is not a network fault, and saying "connection error"
        // would send the operator to check a relay that is answering fine.
        const err = new Error(certPin.describe(e)
          || `Agent ${scheme.toUpperCase()} connection error: ` + e.message);
        if (!configuredScheme && idx + 1 < schemes.length) return tryScheme(idx + 1, err);
        reject(err);
      });
      req.end();
...
class Monitor extends EventEmitter {
  constructor(cfg, opts = {}) {
    ...
    // from "stale" (last known data, yellow) to "offline" (red). Ayarlanabilir
    // (Ayarlar > İzleme > Heartbeat), 1-5 arası.
    this.OFFLINE_AFTER = clampInt(cfg.offlineAfter, 1, 5, 3);
    // Poll başına üst zaman aşımı; 0 = otomatik (pollMs*2, 12-25s).
    this.sshTimeoutMs = clampInt(cfg.sshTimeoutMs, 0, 60000, 0);
    // Izlenecek servis adlari/portlari ayardan gelir; script'in basina yazilir.
    this.watchPrefix = buildWatchPrefix(cfg);
    _sshRetryCount = clampInt(cfg.sshRetryCount, 1, 3, 2);
  ...
  stop() {
    ...
    this.timers.clear();
    for (const p of this.logStreams.values()) { try { typeof p.close === 'function' ? p.close() : p.kill(); } catch {} }
  ...
  updateServers(next) {
    ...
    for (const name of Array.from(this.timers.keys())) {
      if (!keep.has(name)) {
        ...
        this._lastRates?.delete(name);
        this._logCache.delete(name);
  ...
  updateSettings(s) {
    ...
    if (s.offlineAfter != null) this.OFFLINE_AFTER = clampInt(s.offlineAfter, 1, 5, this.OFFLINE_AFTER);
    if (s.sshTimeoutMs != null) this.sshTimeoutMs = clampInt(s.sshTimeoutMs, 0, 60000, this.sshTimeoutMs);
    if (s.sshRetryCount != null) _sshRetryCount = clampInt(s.sshRetryCount, 1, 3, _sshRetryCount);
  ...
  async _poll(server) {
    ...
    try {
      ...
      if (server.agentEnabled && this.connectionMode !== 'ssh') {
        ...
        try {
          applyAgent(await fetchAgent(server));
        } catch (agentErr) {
          if (/token hatali/i.test(agentErr.message)) {
            ...
            // Gecici ag/TLS takilmasi. 143 relay'de 1144 istekle olculdu: sweep
            // basina ~%1, hicbir relay tutarli degil, hep baska makine. Tek hata
            // sari "stale" alarm uretiyor. SSH fallback'ine dusmeden once bir kez
            // daha dene — bagimsiz %1 hata, iki ardisik denemede ~%0.01'e iner.
            // iPhone uygulamasi ayni tekrari zaten yapiyor (AgentClient).
            try {
      ...
      if (this.connectionMode === 'ssh' || !server.agentEnabled || agentTokenFailed) {
        ...
        // boxes under anon CPU load were tripping the old 25s cap and flapping
        // to "stale"), but never more than 2x pollMs so we don't pile up.
        const sshTimeout = this.sshTimeoutMs > 0
          ? Math.max(5000, Math.min(60000, this.sshTimeoutMs))
      ...
      if (/No route to host|Network is unreachable/i.test(errMsg) && !this._hasLocalNetwork()) {
        this._logDebug(`${server.name} skipped — this machine has no network connection; not counted as a relay error`);
        return;
      }
      // Aynı anda filonun büyük kısmı hata veriyorsa sorun relay'lerde değil,
      // bu makinenin ağındadır. Ölçüldü (2026-09-13): aynı filoyu sunucudan
      // yoklayan nöbetçide iki saatlik bir ağ dalgalanması 55 sahte "çevrimdışı"
      // alarmı üretti, relay'lerin hepsi ayaktayken. `_hasLocalNetwork` bunu
      // yakalayamıyor çünkü makine ağsız değil, ağ sadece bozuk.
      if (this._noteFailureAndSuspectNetwork(server.name)) {
        this._logDebug(`${server.name} skipped — fleet-wide failures, treating as local network trouble`);
        return;
      }
      const prev = this.state.get(server.name) || {};
      const fails = (prev.fails || 0) + 1;
      this.state.set(server.name, { ...prev, fails });
      // Card is only "offline" (red) after OFFLINE_AFTER consecutive failures,
      // and "stale" (yellow) one miss earlier. A SINGLE missed poll stays green:
      // olculdu 2026-09-03, 143 sagliklı host uzerinde sweep basina ~%1 gecici
      // hata var (ayni hostlar tek tek yoklaninca ~105ms, %0 paket kaybi) — yani
      // tek kacan poll neredeyse her zaman ag gurultusu, relay arizasi degil.
      // Son iyi degerler ekranda kalir.
      const staleAfter = Math.max(1, this.OFFLINE_AFTER - 1);
      let state = fails >= this.OFFLINE_AFTER ? 'offline'
        : fails >= staleAfter ? 'stale'
          : 'online';
      // Kirmizi, alarm ve auto-fix demek. Oraya gecmeden once bir kez daha sor:
      // olculdu 2026-09-14, 140 relaylik bir filoda gece boyunca uretilen tekil
      // alarmlarin hepsi bir sonraki yoklamada kendiliginden kapandi — yani
      // hicbiri gercek degildi. Bir istegin maliyeti, bosa uyandirmanin
      // maliyetinden ucuz.
      if (state === 'offline' && !this._confirming) {
        this._confirming = true;
        try {
          // Ayni yol: agent acikken agent, degilse SSH.
          if (server.agentEnabled && this.connectionMode !== 'ssh') await fetchAgent(server);
          else await runSsh(server, 'echo OK', 8000);
          this._logDebug(`${server.name} esigi gecti ama dogrulamada cevap verdi — alarm yok`);
          this.state.set(server.name, { ...prev, fails: 0 });
          return;
        } catch {
          // dogrulama da basarisiz: gercekten dusmus, alarm devam
        } finally {
          this._confirming = false;
        }
      }
      this._logDebug(`${server.name} FAIL (${fails}x, ${state}): ${errMsg.replace(/\n/g, ' | ').slice(0, 300)}`);
      // Cache logs from the FIRST failure while SSH may still be reachable, so AI
      // auto-fix has recent logs by the time the relay flips to offline. Tied to the
      // failure count, not the displayed colour: with staleAfter > 1 the first miss
      // no longer shows yellow, but it is still the best moment to grab logs.
      if (state !== 'offline' && !this._logCache.has(server.name)) {
  ...
  /// Son bir tarama penceresinde kaç FARKLI relay hata verdi? Eşiği aşarsa
  /// şüphe penceresi açılır ve o pencere boyunca hatalar sayılmaz — kendi ağ
  /// kesintimiz kimsenin relay'ini kırmızıya itmesin.
  _noteFailureAndSuspectNetwork(name) {
    const now = Date.now();
    const windowMs = Math.max(30000, (this.pollMs || 10000) * 2);
    if (!this._recentFails) this._recentFails = [];
    this._recentFails.push({ name, t: now });
    this._recentFails = this._recentFails.filter(f => now - f.t <= windowMs);
  ...
  _emitAggregate() {
    ...
    // Sum of last-known rates — but ONLY for servers still within the flap
    // window. Offline servers keep their last net counters (for the recovery
    // diff) but _poll never refreshes _lastRates for them, so without this
    // guard a relay that dropped at 5 Mbps keeps adding 5 Mbps to the fleet
    // total forever (inflated tray title / header rate).
    for (const s of this.servers) {
      const st = this.state.get(s.name);
      if (st && (st.fails || 0) >= this.OFFLINE_AFTER) continue;
  ...
  async runCommand(serverName, command, timeoutMs = 30000) {
    ...
    if (!server) return { ok: false, error: 'Server not found: ' + serverName };
    try {
...
printf '\n===ANONRC_END===\n')`;
  ...
  async writeAnonrc(server, content, opts = {}) {
    ...
    // gibi programatik çağrıcılar ve ham IPC bunu atlıyordu.
    const body = String(content || '');
    if (body.trim().length < 20 || !/^\s*(ORPort|Nickname)\b/mi.test(body)) {
...
if [ -n "$ANONRC" ]; then
  ...
  echo "CHECK_ANONRC=err:anonrc not found (/etc/anon/anonrc)"
fi
...
if [ -f /etc/anon/anyone-exit-notice.html ]; then
  ...
  echo "CHECK_EXIT_NOTICE=warn:file missing but DirPortFrontPage is configured in anonrc — create it with: mkdir -p /etc/anon && echo '<html><body>Exit Node</body></html>' > /etc/anon/anyone-exit-notice.html"
else
  echo "CHECK_EXIT_NOTICE=ok:not configured (DirPortFrontPage is not in anonrc)"
fi

# 3. ORPort 9001 dinleniyor mu?
if ss -tlnp 2>/dev/null | grep -q ':9001 '; then
  echo "CHECK_ORPORT=ok:9001 listening"
elif ss -tlnp 2>/dev/null | grep -q ':9001'; then
  echo "CHECK_ORPORT=ok:9001 listening"
else
  echo "CHECK_ORPORT=warn:9001 not listening yet — the service may have just started; wait 1–2 minutes"
fi
...
if [ -z "$SVC_ENABLED_NAME" ]; then
  echo "CHECK_SVC_ENABLED=warn:anon service was not found on the system"
elif echo "$SVC_ENABLED_STATE" | grep -q 'enabled'; then
  echo "CHECK_SVC_ENABLED=ok:$SVC_ENABLED_NAME enabled ($SVC_ENABLED_STATE)"
else
  echo "CHECK_SVC_ENABLED=err:$SVC_ENABLED_NAME — $SVC_ENABLED_STATE (to enable: systemctl enable $SVC_ENABLED_NAME)"
fi
...
if [ -n "$DISK_PCT" ] && echo "$DISK_PCT" | grep -qE '^[0-9]+$'; then
  if [ "$DISK_PCT" -ge 90 ]; then
    echo "CHECK_DISK=err:\${DISK_PCT}% full — clean up disk space"
  elif [ "$DISK_PCT" -ge 75 ]; then
    echo "CHECK_DISK=warn:\${DISK_PCT}% full — monitor it"
  else
    echo "CHECK_DISK=ok:\${DISK_PCT}% full"
  fi
else
  echo "CHECK_DISK=warn:could not read disk information"
fi
...
if command -v anon >/dev/null 2>&1; then
  ...
  echo "CHECK_VERSION=warn:anon command not found (there may be a PATH problem)"
fi
...
`;
    try {
      ...
      const checkLabels = {
        CHECK_ANONRC: 'anonrc file',
        CHECK_EXIT_NOTICE: 'Exit Notice HTML',
        CHECK_ORPORT: 'ORPort 9001',
        CHECK_SVC_ENABLED: 'Service (enabled)',
        CHECK_DISK: 'Disk usage',
        CHECK_VERSION: 'anon version',
      };
      const checks = [];
      for (const line of out.split('\n')) {
  ...
  startLogStream(name) {
    ...
    if (!server) return;
    if (SANDBOX) {
      const stream = sandboxTransport.startLogStream(server, (line) => this.emit('log', { name, line }));
  ...
  stopLogStream(name) {
    ...
    if (p) { try { typeof p.close === 'function' ? p.close() : p.kill(); } catch {} this.logStreams.delete(name); }
  }
}
