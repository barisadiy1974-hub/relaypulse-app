...
const TOP_LEVEL_SECRET_FIELDS = ['claudeApiKey', 'openaiApiKey', 'sshPrivateKey'];
const SERVER_SECRET_FIELDS = ['password', 'agentToken'];

const DEFAULTS = {
  pollMs: 120000,
  logLines: 1000,
  // SSH bağlantı davranışı — Ayarlar > İzleme'den ayarlanır.
  // sshRetryCount: ssh istemcisinin ConnectionAttempts değeri (1-3).
  // sshTimeoutMs: tek poll için üst zaman aşımı; 0 = otomatik (pollMs*2, 12-25s arası).
  // offlineAfter: kaç ardışık başarısızlıktan sonra kart "offline" (kırmızı) olur (1-5).
  sshRetryCount: 2,
  sshTimeoutMs: 0,
  offlineAfter: 2,
  ...
  // sabit listeyle ayni, boylece mevcut kurulumlar aynen calismaya devam
  // eder. Ornek: ['mysterium-node'], ['nginx','postgresql'], ['docker'].
  watchServices: ['anon', 'anon@default', 'anyone', 'anyone-relay', 'tor-anon'],
  // Servis adiyla eslesme olmazsa yedek tespit: bu portlardan biri
  // dinleniyorsa servis ayakta sayilir.
  watchPorts: [9001, 9030, 9050, 9051],
  themeMode: 'dark',
  languageMode: 'en',
  alarmEnabled: true,
  alarmSound: 'Hero',
  ...
  claudeApiKey: '',
  openaiApiKey: '',
  // MAS sandbox SSH private key. Its plaintext only ever lives in Electron's
  // safeStorage-backed secret vault, never in anyone-monitor.json.
  sshPrivateKey: '',
  // Public SSH host-key fingerprints accepted on first use by the sandbox
  ...
  anyoneOrPort: 0,
  // Mac App Store lifetime unlock. Only ever written from a StoreKit
  // transaction or a restore (see src/iap.js), never from the UI.
  masPurchased: false,
  // Replaces the fleet with an invented one so the app can be looked around
  // without any servers. See src/demo-fleet.js.
  demoMode: false,
...
class Config {
  ...
  setSshHostKey(serverKey, fingerprint) {
    const cfg = this.load();
    cfg.sshHostKeys = { ...(cfg.sshHostKeys || {}), [String(serverKey)]: String(fingerprint) };
    this.save(cfg);
  }
