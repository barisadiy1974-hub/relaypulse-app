...
# Hangi systemd servisinin ayakta olduguna bakilacak, ve hangi portlarin
# dinlenmesi "ayakta" sayilacak. Makine metrikleri (CPU/RAM/disk/ag/uptime)
# her Linux sunucusunda ayni sekilde okunuyor; bu ayar sadece servis durumu
# icin. Varsayilanlar eski sabit listeyle birebir ayni, boylece kurulu
# ajanlar davranis degistirmez.
def _envlist(name, default):
    raw = os.environ.get(name, '') or ''
    vals = [v for v in re.split(r'[\s,]+', raw) if v]
    return vals or default

SERVICES = _envlist('AGENT_SERVICES',
                    ['anon', 'anon@default', 'anyone', 'anyone-relay', 'tor-anon'])
SERVICES = [v for v in SERVICES if re.match(r'^[A-Za-z0-9@._-]+$', v)]
PORTS = [p for p in _envlist('AGENT_PORTS', ['9001', '9030', '9050', '9051'])
...
class Server(http.server.ThreadingHTTPServer):
    ...
    def get_request(self):
        ...
        # ThreadingHTTPServer burada yardim etmez, cunku is parcaciklari ancak
        # accept() donduKten SONRA devreye girer.
